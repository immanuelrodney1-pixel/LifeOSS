        ...
        <div className="py-6 flex flex-col gap-8 pb-32">
            <header className="flex items-center justify-between">
                <div className="flex flex-col gap-1">
                    ...
                    <p className="text-accent text-sm">Review your patterns and distribution.</p>
                </div>
                <div className="flex items-center gap-2 bg-secondary/50 p-1 rounded-full border border-accent/10">
                    ...
                    <span className="text-xs font-bold px-2">Mar 3 – Mar 9</span>
                    <Button variant="ghost" size="sm" className="h-8 w-8 p-0 rounded-full"><ChevronRight size={16} /></Button>
                </div>
            </header>

            <section className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <Card className="p-6 bg-primary text-secondary flex flex-col gap-4">
                    <div className="flex items-center gap-2 opacity-80">
                        <TrendingUp size={18} />
                        <span className="text-xs font-semibold uppercase tracking-widest">Weekly Focus</span>
                    </div>
                    <p className="text-3xl font-bold">Deep Work</p>
                    <div className="flex flex-col gap-2 mt-2">
                        <div className="flex justify-between text-xs opacity-80 mb-1">
                            <span>22.5 hours total</span>
                            <span>42% of logged time</span>
                        </div>
            ...
            <section className="flex flex-col gap-4">
                ...
                <div className="flex flex-col gap-3">
                    {days.map((day) => (
                        <Card key={day.name} className="p-4 flex items-center justify-between hover:border-primary/20 transition-all cursor-pointer">
                            <div className="flex items-center gap-4">
                                <div className="flex flex-col items-center justify-center w-12 py-2 rounded-xl bg-accent/5 border border-accent/10">
                                    <span className="text-xs font-bold uppercase tracking-tighter">{day.name}</span>
                                    <span className="text-[10px] text-accent">{day.date.split(' ')[1]}</span>
                                </div>
                                <div>
                                    <div className="flex items-center gap-2">
                                        <p className="font-bold">{day.hours} logged</p>
                                        <Badge variant="secondary" className="text-[10px] h-4 leading-none">{day.top}</Badge>
                                    </div>
                                    <p className="text-xs text-accent mt-0.5">Primary State: {day.mood} ({day.intensity}/10)</p>
                                </div>
                            </div>
                            < ChevronRight size={18} className="text-accent/30" />
                        </Card>
                    ))}
                </div>
            </section>
